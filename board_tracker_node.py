...
ROS node tracking 8x8 reed-switch chess board.
Topics in : /chess/game_start, /chess/board_state
Topics out: /chess/move, /chess/promotion_request, /chess/status, /chess/board_occupancy
Modes: WAITING | ACTIVE | MONITOR | LOCKED
"""
import json
import threading
import time
...
# ============================================================================
ROW_PINS = [4, 17, 27, 22, 5, 6, 13, 19]      # row 0 = rank 8 ... row 7 = rank 1
COL_PINS = [26, 21, 20, 16, 12, 25, 24, 23]   # col 0 = file a ... col 7 = file h
SETTLE_TIME_S = 0.0005                         # row-drive settle
...
def occupancy_from_chess(board):
    ...
    for r in range(8):
        for c in range(8):
            if board.piece_at(chess.square(c, 7 - r)) is not None:
                occ[r][c] = 1
    return occ
...
class SimulatedSensorBoard:
    """Software simulator with same interface as the real GPIO board."""
...
class RPiSensorBoard:
    """8x8 reed-switch matrix scanner over RPi.GPIO.

    Wiring: rows are outputs (idle HIGH), columns are inputs with pull-up.
    Scanning a row pulls it LOW; a closed reed switch pulls its column LOW
    -> occupied square reports 1.
    """
        ...
        with self._lock:
            for r, row_pin in enumerate(ROW_PINS):
                ...
                for c, col_pin in enumerate(COL_PINS):
                    new[r][c] = 0 if GPIO.input(col_pin) else 1
                GPIO.output(row_pin, GPIO.HIGH)
        return new

    # No-ops on real hardware (sensor reflects the physical world).
    def set_square(self, sq, value):
        pass
...
class MoveDetector:
    @staticmethod
    def squares_touched_by_move(board, move):
    ...
    def classify(cls, chess_board, current_occ, anchor_occ):
        """kind in: clean | complete | promotion | pending | anomaly"""
        ...
        for m in legal:
            ...
            if diff_set.issubset(touched) and \
                    cls._consistent_with_move(chess_board, m, current_occ, anchor_occ):
    ...
    def _consistent_with_move(cls, board, move, current_occ, anchor_occ):
        ...
        for r in range(8):
            for c in range(8):
                a, f, cur = anchor_occ[r][c], final_occ[r][c], current_occ[r][c]
                if cur == a or cur == f:
                    continue
                if cur == 0 and f == 1:
...
class BoardTrackerNode:
    ...
    def __init__(self):
        ...
        if GPIO_AVAILABLE:
            self.sensor = RPiSensorBoard()
            print("[INFO] Using RPi.GPIO reed-switch matrix.")
        else:
            self.sensor = SimulatedSensorBoard(occupancy_from_chess(self.chess_board))
            print("[INFO] RPi.GPIO not available - running with software simulator.")
        ...
        if ROS_AVAILABLE:
            ...
            self.pub_promo = rospy.Publisher(self.TOPIC_PROMOTION_REQUEST, String, queue_size=10)
    ...
    def _on_game_start(self, msg):
        ...
        # Sync simulator only; real GPIO sensor reflects physical state.
    ...
    def _on_board_state(self, msg):
        ...
        # board_state is the source of truth: sync simulator only.
        if isinstance(self.sensor, SimulatedSensorBoard):
            self.sensor.load_from_chess_board(new_board)
            if self.flipped:
                with self.sensor._lock:
                    self.sensor._board = flip_occupancy_180(occupancy_from_chess(new_board))
    ...
    def _update_mode(self):
        ...
        if self.locked_uci is not None or self.pending_promotion_from_to is not None:
            ...
            return
        self.mode = Mode.ACTIVE if self.chess_board.turn == self.human_color else Mode.MONITOR
    ...
    def _publish_move(self, uci):
        ...
        # Optimistic local push - board_state will confirm or override.
        try:
    ...
    def _read_sensor(self):
        """Read sensor and rotate 180 if board is flipped (human plays black)."""
        raw = self.sensor.scan()
        return flip_occupancy_180(raw) if self.flipped else raw
    ...
    def _dispatch_stable(self, current_occ):
        if self.mode in (Mode.WAITING, Mode.LOCKED):
            return
    ...
    def _active_classify(self, current_occ):
        ...
        if kind == 'pending':
            ...
            if self.LIFT_TIMEOUT_S < elapsed < self.LIFT_TIMEOUT_S + 0.5:
...
"""
    ...
    def _simulate_physical_move(self, from_sq, to_sq, promo):
        ...
        # In MONITOR/RvR: apply directly (simulates robot arm).
        if self.mode != Mode.ACTIVE:
        ...
        # ACTIVE: stage realistic sensor changes for the scan loop to detect.
        try:
        ...
        wait_step = max(2.0 / self.SCAN_RATE_HZ, 0.05)
