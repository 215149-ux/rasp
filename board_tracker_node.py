#!/usr/bin/env python3
"""
board_tracker_node.py - Physical Board Tracker (Raspberry Pi)
═══════════════════════════════════════════════════════════════════
...
ROS Topics:
  Publishers : /chess/move, /chess/promotion_request, /chess/status, /chess/pause
  Subscribers: /chess/game_start, /chess/board_state, /chess/turn_signal,
               /chess/game_stop, /chess/pause
"""
...
def rc_to_sq(row, col):
    """Physical (row,col) -> chess square. Cols mirrored: phys col 0 = file h."""
    return FILES[7 - col] + str(8 - row)


def occupancy_from_chess(board):
    """chess.Board -> 8x8 occupancy with mirrored columns."""
    occ = [[0] * 8 for _ in range(8)]
    for r in range(8):
        for c in range(8):
            sq = chess.square(7 - c, 7 - r)
...
class GPIOSensorBoard:
    """Reed-switch matrix: rows are inputs (pull-up), cols are driven LOW one at a time."""
...
class MoveDetector:
    ...
    def classify(cls, chess_board, current_occ, anchor_occ):
        ...
        if complete:
            ...
            return {'kind': 'anomaly', 'disappeared': disappeared,
                    'appeared': appeared, 'reason': 'ambiguous'}

        diff_set = set(disappeared) | set(appeared)
        partial = []
        for m in legal:
            touched = cls.squares_touched(chess_board, m)
            if diff_set.issubset(touched) and \
                    cls._consistent(chess_board, m, current_occ, anchor_occ):
                partial.append(m)
        if partial:
            return {'kind': 'pending', 'partial_for': partial,
                    'disappeared': disappeared, 'appeared': appeared}
        return {'kind': 'anomaly', 'disappeared': disappeared,
                'appeared': appeared, 'reason': 'no_match'}
    ...
    def _consistent(cls, board, move, current_occ, anchor_occ):
        ...
        for r in range(8):
            for c in range(8):
                a, cur, f = anchor_occ[r][c], current_occ[r][c], final[r][c]
                if cur == a or cur == f:
                    continue
                if cur == 0 and f == 1:
                    continue
...
class BoardTrackerRPi:
    ...
    def __init__(self):
        ...
        if ROS_AVAILABLE:
            ...
            self.pub_move = self.pub_promo = self.pub_status = self.pub_pause = None
    ...
    def _on_game_start(self, msg):
        ...
        self._print(f"✅ Game started! mode={self.game_mode} "
                    f"tracker_active={self.tracker_active} → {self.mode}")
        print_matrix(self._anchor_occ, "ANCHOR (initial)")

    def _on_board_state(self, msg):
        self._print("📥 /chess/board_state received")
        try:
            p = json.loads(msg.data)
        except Exception:
            return
        ...
        if not self.game_active:
            self._print("   ⚡ Auto-activating game from board_state")
            self.game_active = True
    ...
    def _on_turn_signal(self, msg):
        ...
        if not self.game_active and self.tracker_active:
            self._print("   ⚡ Auto-activating game (game_start missed)")
            self.game_active = True
            self.game_mode = 'HvR'
        ...
        if fen:
            try:
                ...
                if new_board.fen() != self.chess_board.fen():
                    ...
                    self._anchor_occ = occupancy_from_chess(self.chess_board)
                    self._print("   anchor updated from FEN")
        ...
        if self.mode == Mode.ACTIVE:
            self._print(f"\n{'─'*50}")
            self._print("👉 YOUR TURN! Move your piece then press [ENTER]")
            self._print(f"{'─'*50}")
    ...
    def _update_mode(self):
        if not self.game_active or self.paused or self.game_mode == 'RvR':
            self.mode = Mode.WAITING
    ...
    def _publish_move(self, uci):
        ...
        self.tracker_active = False
        self._update_mode()
        self._print("   → LOCKED. Waiting for engine response...")
    ...
    # ─── Button Listener (ENTER = push button) ────────────────────
    ...
    def _main_loop(self):
        while not self._shutdown.is_set():
            ...
            # Paused due to error → ENTER means "fixed, resume"
            if self.paused and self.game_active:
                self._button_pressed.clear()
                if self._button_pressed.wait(timeout=0.3):
            ...
            self._button_pressed.clear()
            if not self._button_pressed.wait(timeout=0.3):
                continue
            ...
            if kind == 'complete':
                ...
                self._print("   ⏳ Incomplete move — you might still be moving.")
                self._print("      Finish your move and press [ENTER] again.")
            elif kind == 'anomaly':
                ...
                self._print("      ⏸  PAUSING GAME — fix the board then press [ENTER]")
                self._pub(self.pub_status,
                          "⚠️ Board error detected — game paused. Fix pieces and resume.")
            ...
            if self.mode == Mode.LOCKED and self.locked_uci:
                if time.time() - self.locked_time > self.LOCKED_TIMEOUT_S:
                    self._print("⏰ LOCKED timeout — unlocking.")
                    self.locked_uci = None
                    self._update_mode()

    def _stable_read(self, attempts=5, delay=0.05):
        """Multi-read consensus to debounce sensor noise."""
    ...
    def verify_initial(self):
        ...
        while not self._shutdown.is_set():
            ...
            if time.time() > deadline:
                ...
                self._print(f"⚠️  TIMEOUT: Continuing with {pieces}/32 pieces")
    ...
    def run(self):
        self._print("╔══════════════════════════════════════════════════════╗")
        self._print("║   Board Tracker — BUTTON MODE (Press ENTER)         ║")
        self._print("╚══════════════════════════════════════════════════════╝")
        ...
        self._print("📡 Subscribed: game_start, board_state, turn_signal, game_stop, pause")
        self._print("📢 Publishing: /chess/move, /chess/promotion_request\n")
        self._print("┌─────────────────────────────────────────────────┐")
        self._print("│  HOW TO USE:                                    │")
        self._print("│  1. Wait for 'YOUR TURN' message                │")
        self._print("│  2. Move your piece on the physical board       │")
        self._print("│  3. Press [ENTER] to confirm your move          │")
        self._print("└─────────────────────────────────────────────────┘\n")
        self._print("⏳ Waiting for /chess/game_start...\n")
